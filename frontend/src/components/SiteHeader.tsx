...
export default function SiteHeader({ role = "student", onRoleChange }: { role?: Role; onRoleChange?: (role: Role) => void }) {
  ...
  const isDemoWorkspace = location.pathname.startsWith("/demo");
  return (
    <>
      <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-[#F8FAF6]/95 backdrop-blur-md" data-testid="site-header">
        <div className="mx-auto flex h-[76px] max-w-7xl items-center justify-between gap-5 px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            ) : <Link to="/login" className="hidden text-sm font-semibold text-[#0B3C2A] sm:inline-flex" data-testid="header-login-link">Secure sign in</Link>}
            <Link to={isWorkspace ? "/app/student" : "/login"} className="inline-flex h-10 items-center rounded-md bg-[#0B3C2A] px-3.5 text-sm font-semibold text-white shadow-sm transition-transform hover:-translate-y-0.5 hover:bg-[#072B1E]" data-testid="header-get-started-button">{isWorkspace ? "Workspace" : "Get started"}</Link>
